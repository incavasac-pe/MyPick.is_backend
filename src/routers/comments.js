...
router.post('/register_like_comments', async (req, res) => {
    ...
    const {id_pick, comentario_id, email} = req.body;
    ...
    existLikeComments = await new Comments().getLikesbyidpick(id_user, id_pick);
        let arrayInsert = []
        arrayInsert = existLikeComments.rowCount > 0 ? existLikeComments.rows[0].coment_id_like  : arrayInsert
        const newArray = validarNumeroExistente(arrayInsert, comentario_id)

        if (newArray) {
            const index = arrayInsert.indexOf(comentario_id.toString());
            if (index > -1) {
                arrayInsert.splice(index, 1);
            }
        } else {
            arrayInsert.push(comentario_id.toString())
        } 
         await new Comments().insertLikecomments_user(id_pick,arrayInsert,id_user)

    const pick_update = await new Comments().updateLikesComments(id_pick, comentario_id,newArray)
